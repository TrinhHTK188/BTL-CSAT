        ...
        try:
            for conn in process.info['connections']:
                if target_address in conn.raddr:
                    ...
                    return_code, out, err = run_subprocess(taskkill_command)

                    if return_code == 0:
                        print(f"Đã dừng tiến trình có PID {
                              pid}, kết nối đến {target_address}")
                    else:
                        print(f"Lỗi khi dừng tiến trình có PID {
                              pid}: {err.decode()}")
