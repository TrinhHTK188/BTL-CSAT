    ...
    if DNS in packet and packet.qr == 0:
        ...
        if app != "Không xác định ứng dụng":
            ...
            if is_within_allowed_time():
                if (app_to_type(app) == "Social Media and Communication" or app_to_type(app) == "Entertainment"):
                    kill_processes(app)
            print(f"{domain} : {app}")
        else:
            ...
            if ip_address != "Không thể tìm thấy địa chỉ IP cho tên miền.":
                ...
                if app_from_ip != "Không xác định ứng dụng":
                    ...
                    if is_within_allowed_time() and (app_to_type(app_from_ip) == "Social Media and Communication" or app_to_type(app_from_ip) == "Entertainment"):
